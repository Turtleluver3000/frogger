...
class App:
	...
	def event(self, event):
		...
		if self.state == 'PLAYING':
			...
			if event.type == KEYDOWN and event.key == K_UP:
				self.frog.move(0, -1)
			if event.type == KEYDOWN and event.key == K_DOWN and self.frog.y < 380:
				self.frog.move(0, 1)
	...
	def draw(self):
		...
		if self.state == 'START':
			...
			self.draw_text("Press ENTER to start playing.ESC to quit", g_vars['width']/2, g_vars['height']/2 + 15, 'center')
